export async function createSnapshot(instanceName: string, name?: string, stateful?: boolean) {
...
export async function deleteSnapshot(instanceName: string, snapshotName: string) {
...
export async function restoreSnapshot(instanceName: string, snapshotName: string) {
    const shortName = snapshotName.split("/").pop() || "";
...
export async function renameSnapshot(instanceName: string, snapshotName: string, newName: string) {
    const shortName = snapshotName.split("/").pop() || "";
    const res = await fetch(
        `/1.0/instances/${instanceName}/snapshots/${shortName}`,
        {
